...
class Color:
  def __init__(self, rgb):
    ...
    self.green = rgb[1]
    self.blue = rgb[2]
    self.hexcode = '#{:02x}{:02x}{:02x}'.format(self.red, self.green, self.blue)
    self.brightness_key = self.red + self.green + self.blue
    self.saturation_key = max(self.red, self.green, self.blue) - min(self.red, self.green, self.blue)

# Get image to pull colors from.
parser = argparse.ArgumentParser()
parser.add_argument('image_file_path', metavar='image_file_path',
help='path to image file')
image_file_path = parser.parse_args().image_file_path

image_size = 100
image = Image.open(image_file_path).resize([image_size, image_size], Image.LANCZOS)
...
highlights = colors[::math.floor(len(colors) / 5)].sort(key=lambda color: color.saturation_key)
midtones = colors[math.floor(len(colors) / 5):0]
shadows = colors[math.floor(len(colors) / 5):-1] #.sort(key=lambda color: color.saturation_key)
